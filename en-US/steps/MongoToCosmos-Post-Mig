...
Until MongoDB 3.6, there was no concept of a session consistency. Therefore, if you set a MongoDB API account to use session consistency, the consistency is downgraded to eventual when using MongoDB APIs. Is this different now since Mongo has session consistency? 

    | Azure Cosmos DB Default Consistency Level | Azure Cosmos DB Mongo API (3.4) |
    | ----------------------------------------- | ------------------------------- |
    | Eventual                                  | Eventual                        |
